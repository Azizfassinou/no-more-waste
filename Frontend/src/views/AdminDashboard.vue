<template>
  <div class="dashboard">
    <h1>{{ $t('dashboards.adminTitle') }}</h1>

    <div class="tabs">
      <button :class="{ active: tab === 'users' }" @click="tab = 'users'">{{ $t('dashboards.tabs.users') }}</button>
    ...
    <div v-if="tab === 'users'">
      <h2 class="section-title">{{ $t('dashboards.createStaffTitle') }}</h2>
      <div class="form-section">
        <div class="form-row">
          <div class="form-group"><label>{{ $t('auth.firstname') }}</label><input v-model="newStaff.firstname" /></div>
          <div class="form-group"><label>{{ $t('auth.lastname') }}</label><input v-model="newStaff.lastname" /></div>
        </div>
        <div class="form-row">
          <div class="form-group"><label>{{ $t('auth.email') }}</label><input v-model="newStaff.email" type="email" /></div>
          <div class="form-group"><label>{{ $t('auth.password') }}</label><input v-model="newStaff.password" type="password" /></div>
        </div>
        <div class="form-group"><label>{{ $t('dashboards.roleTitle') }}</label><input v-model="newStaff.employee_role" placeholder="Gestionnaire logistique..." /></div>
        <button class="btn btn-primary" @click="createStaff">{{ $t('dashboards.createStaffBtn') }}</button>
      </div>

      <div style="display: flex; justify-content: space-between; align-items: center; margin-top:2rem; margin-bottom: 1rem;">
        <h2 class="section-title" style="margin:0;">{{ $t('dashboards.allUsersTitle') }}</h2>
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          <label style="font-size: 0.88rem; font-weight: 600; color: var(--text-muted, #64748b);">{{ $t('dashboards.filterByRole') }} :</label>
          <select v-model="roleFilter" style="padding: 0.4rem 0.8rem; border-radius: 6px; border: 1px solid #cbd5e1; font-size: 0.88rem; background: #fff;">
            <option value="">{{ $t('dashboards.allRoles') }}</option>
      ...
      <div v-if="filteredUsers.length === 0" class="empty-state">Aucun utilisateur trouvé pour ce rôle</div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="u in filteredUsers" :key="u.ID">
              <td>#{{ u.ID }}</td>
              <td>{{ u.firstname }} {{ u.last_name }}</td>
              <td>{{ u.email }}</td>
              <td><span class="badge badge-blue">{{ u.role }}</span></td>
              <td>
                <select v-model="userRoleEdit[u.ID]" style="font-size:0.8rem;">
                  <option value="client">client</option>
                  <option value="merchant">merchant</option>
                  <option value="volunteer">volunteer</option>
                  <option value="staff">staff</option>
                ...
                <button class="btn btn-primary btn-sm" style="margin-left:4px;" @click="updateUserRole(u.ID)">Changer</button>
              </td>
              <td>
                <button class="btn btn-danger btn-sm" @click="deleteUser(u.ID)">Supprimer</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-if="tab === 'services'">
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label>Bénévole Animateur (Tous bénévoles)</label>
    ...
    <div v-if="tab === 'rounds'">
      ...
      <div class="form-section">
        ...
        <div class="form-group"><label>Notes</label><input v-model="newRound.notes" placeholder="Instructions spéciale..." /></div>
...
export default {
  ...
  data() {
    return {
      ...
      users: [],
      userRoleEdit: {},
      newStaff: { firstname: '', lastname: '', email: '', password: '', employee_role: '' },
      pendingMerchants: [],
      volunteers: [],
      allSkills: [],
      skillAssign: {},
  ...
  methods: {
    ...
    loadAll() { this.loadUsers(); this.loadMerchants(); this.loadVolunteers(); this.loadSkills(); this.loadServices(); this.loadRounds(); this.loadDeliveries() },
    ...
    async loadUsers() {
      try {
        ...
        this.users.forEach(u => { this.userRoleEdit[u.ID] = u.role })
      } catch(e) { this.users = [] }
    },
    async createStaff() {
      try {
        await api.post('/admin/staff', this.newStaff)
        this.flash('Compte Staff créé avec succès', 'ok')
        this.newStaff = { firstname: '', lastname: '', email: '', password: '', employee_role: '' }
        this.loadUsers()
      } catch(e) { this.flash(e.response?.data?.error || 'Erreur', 'err') }
    },
    async updateUserRole(userId) {
      const newRole = this.userRoleEdit[userId]
      try {
        await api.put('/admin/users/' + userId, { role: newRole })
        this.flash('Rôle mis à jour', 'ok')
        this.loadUsers()
      } catch(e) { this.flash('Erreur', 'err') }
    },
    async deleteUser(userId) {
      if (!confirm('Voulez-vous vraiment supprimer cet utilisateur ?')) return
      try {
        await api.delete('/admin/users/' + userId)
        this.flash('Utilisateur supprimé', 'ok')
        this.loadUsers()
      } catch(e) { this.flash('Erreur', 'err') }
    },
    async loadMerchants() {
